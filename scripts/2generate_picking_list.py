...
if getattr(sys, 'frozen', False):
    # Save in output/plocklistor subdirectory
    ...
    LEVERANSFREKVENS_PATH = PROJECT_ROOT / 'data' / \
        'parametrar' / 'Leveransfrekvens.csv'
    PICKING_LIST_RESULTS_PATH = PROJECT_ROOT / 'output' / \
        'plocklistor' / 'picking_list_results.csv'
else:
...
def find_latest_file(pattern, directory):
    ...
    if not files:
        ...
        if existing_files:
            print(
                f"  Hittade {len(existing_files)} CSV-filer i {directory}, men inga matchar mönstret {pattern}")
            print(f"  Exempel filer: {[f.name for f in existing_files[:5]]}")
        else:
            print(
                f"  Katalogen {directory} är tom eller innehåller inga CSV-filer")
    ...
    try:
        ...
        if store_col is None or freq_col is None:
            ...
            if len(df.columns) >= 2:
                ...
                print(
                    f"  Varning: Kunde inte hitta kolumnnamn, använder: '{store_col}' och '{freq_col}'")
            else:
                print(
                    f"  Fel: CSV-filen har för få kolumner. Förväntar minst 2 kolumner.")
                return {}
            ...
            if pd.notna(butik_raw):
                ...
                if butik:  # Hoppa över tomma rader
                    try:
                        ...
                        print(
                            f"  Varning: Kunde inte tolka frekvens för '{butik}': {freq_raw}")
...
def load_unit_mapping(file_path):
    ...
    print(
        f"  Laddade enhetsmappning för {len(unit_mapping)} produkt-butik-kombinationer")
    return unit_mapping
    ...
    for store_name in stores:
        ...
        for product_name in products:
            ...
            try:
                ...
                print(
                    f"    Varning: Kunde inte prognostisera för {product_name}: {e}")
                predicted_sales = 0.0
        ...
        print(
            f"  Processade {len(store_results)} produkter med matchande lagerdata")
        ...
        if len(store_results) > 0:
            ...
            store_df.to_csv(str(output_file_csv), index=False,
            ...
            if OPENPYXL_AVAILABLE:
                ...
                with pd.ExcelWriter(str(output_file_xlsx), engine='openpyxl') as writer:
                    store_df.to_excel(
                        writer, sheet_name='Plocklista', index=False)
            ...
            print(
                f"  [OK] Sparade plocklista med {len(store_df)} produkter till {output_file_csv}")
            if OPENPYXL_AVAILABLE:
                print(f"  [OK] Sparade Excel-fil: {output_file_xlsx}")
            print(
                f"    Totalt påfyllningsbehov: {store_df['Påfyllningsbehov'].sum():.1f} enheter")
...
def main():
    ...
    if len(results) > 0:
        ...
        print(
            f"Totalt antal enheter att fylla på: {results_df['Påfyllningsbehov'].sum():.1f}")

        # Formatera data
        # Produkt_ID ska vara heltal (inte float)
        if 'Produkt_ID' in results_df.columns:
            results_df['Produkt_ID'] = results_df['Produkt_ID'].fillna(
                0).astype(int)
        ...
        if 'Produktkod' in results_df.columns:
            def format_product_code(val):
                ...
                try:
                    ...
                    return str(val)
            results_df['Produktkod'] = results_df['Produktkod'].apply(
                format_product_code)

        # Prognosticerad_försäljning och Påfyllningsbehov ska ha 1 decimal
        if 'Prognosticerad_försäljning' in results_df.columns:
            results_df['Prognosticerad_försäljning'] = results_df['Prognosticerad_försäljning'].round(
                1)
        if 'Påfyllningsbehov' in results_df.columns:
            results_df['Påfyllningsbehov'] = results_df['Påfyllningsbehov'].round(
                1)
        # Saldo och Varningsgräns kan också ha 1 decimal för konsistens
        if 'saldo_denna_butik' in results_df.columns:
            results_df['saldo_denna_butik'] = results_df['saldo_denna_butik'].round(
                1)
        ...
        results_df.to_csv(str(PICKING_LIST_RESULTS_PATH),
        ...
        if OPENPYXL_AVAILABLE:
            ...
            with pd.ExcelWriter(str(excel_path), engine='openpyxl') as writer:
                results_df.to_excel(
                    writer, sheet_name='Plocklistor', index=False)
