...
if getattr(sys, 'frozen', False):
    PICKING_LIST_PATH = PROJECT_ROOT / 'output' / \
...
def _forecast_weeks_best_model(daily_sales, first_future_monday, future_weeks, today):
    ...
    train_ds['week'] = train_ds['date'].dt.to_period(
    ...
    fdf['week_start'] = fdf['date'] - \
        pd.to_timedelta(fdf['date'].dt.dayofweek, unit='D')
...
def get_weekly_sales_history(product_df, store_name, product_name, weeks_back=52):
    ...
    product_sales['week_year'] = product_sales['year'].astype(
        str) + '-W' + product_sales['week_number'].astype(str).str.zfill(2)

    # Aggregera per vecka
    weekly_sales = product_sales.groupby(['week_start', 'week_number', 'year', 'week_year'])[
        'quantity'].sum().reset_index()
    weekly_sales = weekly_sales.sort_values('week_start')

    # Begränsa till senaste X veckor, always extend to the current week
    if len(weekly_sales) > 0:
        ...
        # Skapa komplett serie av veckor från cutoff till latest (includes current week)
        all_weeks = pd.date_range(
            start=cutoff_date, end=latest_week, freq='W-MON')
        ...
        week_year_str = [f"{y}-W{w:02d}" for y,
                         w in zip(years.values, week_numbers.values)]
...
def create_product_page(fig, sales_history, product_info, predicted_sales, weekly_forecast, unit='st'):
    ...
    if len(sales_history) > 0:
        ...
        ax_main.set_ylabel(
            f'Försäljning ({unit})', fontsize=12, fontweight='bold')
        ...
        y_max_history = sales_history['quantity'].max() if len(
            sales_history) > 0 else 0
        y_max_forecast = weekly_forecast['quantity'].max() if len(
            weekly_forecast) > 0 else 0
        ...
        ax_main.set_title('Försäljningshistorik',
    ...
    for store_name in stores:
        ...
        with PdfPages(str(pdf_filename)) as pdf:
            for idx, (_, product_row) in enumerate(store_products.iterrows(), 1):
                ...
                product_name_display = str(product_row[product_name_col])[
                    :50] if product_name_col in product_row.index else 'Okänt produkt'
                print(
                    f"    Sidan {idx}/{len(store_products)}: {product_name_display}...")

                # Hämta försäljningshistorik för denna produkt
                product_name_in_picking = product_row[product_name_col] if product_name_col in product_row.index else product_row.get(
                    'product_name', '')
                ...
                if matched_product_name:
                    ...
                    matched_sales = store_sales[store_sales['name']
                    ...
                    if len(product_sales_df) > 0:
                        if 'date' in product_sales_df.columns:
                            ...
                            if not pd.api.types.is_datetime64_any_dtype(product_sales_df['date']):
                                product_sales_df['date'] = pd.to_datetime(
                                    product_sales_df['date'])
                        elif 'updated' in product_sales_df.columns:
                            # Fallback till updated om date saknas
                            product_sales_df['date'] = pd.to_datetime(
                                product_sales_df['updated'])
                ...
                if 'expected_stock_after_sales' not in product_info:
                    ...
                    predicted_sales = product_info.get(
                        'predicted_sales_units', 0)
                    product_info['expected_stock_after_sales'] = max(
                        0, current_stock - predicted_sales)

                # Skapa sidan
                predicted_sales_value = product_info.get(
                    'predicted_sales_units', 0)
                delivery_frequency_days = product_info.get(
